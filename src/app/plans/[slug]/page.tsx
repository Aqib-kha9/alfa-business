...
export default function PlanDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (matchedPlan) {
      setSelectedImage(matchedPlan.images?.[0] || matchedPlan.image);
    }
  }, [slug]);

  if (!plan) {
    return <div className="text-center py-10 text-gray-600">Loading plan details...</div>;
  }

  const handlePrev = () => {
    if (plan.images && plan.images.length > 1 && selectedImage) {
      const currentIndex = plan.images.findIndex((img) => img === selectedImage);
  ...
  const handleNext = () => {
    if (plan.images && plan.images.length > 1 && selectedImage) {
      const currentIndex = plan.images.findIndex((img) => img === selectedImage);
      const nextIndex = (currentIndex + 1) % plan.images.length; // ✅ FIXED LINE
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
      ...
      <div className="relative">
        <div className="mb-4 relative">
          ...
          {plan.images?.length && plan.images.length > 1 && (
            <>
